...
class ResumeVoiceBroadcastUseCase @Inject constructor(
    ...
    suspend fun execute(roomId: String) {
        ...
        Timber.d("Resume voice broadcast requested")
        ...
        when (lastVoiceBroadcastEvent?.content.toModel<MessageVoiceBroadcastInfoContent>()?.voiceBroadcastState) {
            ...
            VoiceBroadcastState.RESUMED -> {
                Timber.d("Cannot resume voice broadcast: already running")
                // TODO return errors
            }
            VoiceBroadcastState.STOPPED -> {
                Timber.d("Cannot resume voice broadcast: stopped")
            }
            null -> {
                Timber.d("Cannot resume voice broadcast: no voice broadcast found")
            }
        }
    }
