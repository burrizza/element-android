...
import im.vector.app.features.voicebroadcast.model.isStarted
...
class StopVoiceBroadcastUseCase @Inject constructor(
    ...
    suspend fun execute(roomId: String) {
        ...
        Timber.d("Stop voice broadcast requested")
        ...
        val lastVoiceBroadcastState = lastVoiceBroadcastEvent?.content.toModel<MessageVoiceBroadcastInfoContent>()?.voiceBroadcastState
        if (lastVoiceBroadcastState?.isStarted() != true) {
            Timber.d("Cannot stop voice broadcast: no started found")
            return // TODO return an error
        }

        val newContent = MessageVoiceBroadcastInfoContent(
                relatesTo = RelationDefaultContent(RelationType.REPLACE, lastVoiceBroadcastEvent?.eventId),
